...
const PEARL_WALLET = '0xC04C654359f1be91e6b47c9da5F3259b62622Fb4';
const PUMPSPACE_URL = `https://pumpspace.io/wallet/detail?account=${PEARL_WALLET}`;
...
export default function PearlAnalysisPage() {
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem('lastTab', 'wallet');
  }, []);

  const { data: history, isLoading: historyLoading } = useQuery({
    queryKey: ['wallet-history', PEARL_WALLET],
    queryFn: () => fetchWalletHistory(PEARL_WALLET),
  });

  const { data: currentAssets } = useQuery({
    queryKey: ['wallet-current', PEARL_WALLET],
    queryFn: () => fetchCurrentAssets(PEARL_WALLET),
  });
      ...
      <header className="border-b border-slate-700/50 backdrop-blur-sm">
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-4 md:py-5">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-white mb-1">PEARL 펀드</h1>
              <code className="text-xs md:text-sm text-slate-400 font-mono">
                {formatAddress(PEARL_WALLET)}
              </code>
            </div>
            <a
              href={PUMPSPACE_URL}
