...
export default function BuybackAnalysisPage() {
  ...
  // 모든 지갑 주소
  const allAddresses = BUYBACK_WALLETS.map(w => w.address);

  // 현재 자산 조회
  const { data: currentAssetsData } = useQuery({
    queryKey: ['buyback-current'],
  ...
  const selectedWallet = selectedTab === 'total' 
    ? null 
    : BUYBACK_WALLETS.find(w => w.id === selectedTab);

  // 히스토리 조회 (개별 지갑)
  const { data: history, isLoading: historyLoading } = useQuery({
    queryKey: ['buyback-history', selectedWallet?.address],
  ...
  const { data: allHistories, isLoading: allHistoriesLoading } = useQuery({
    queryKey: ['buyback-all-histories'],
    queryFn: async () => {
      const results: Record<string, HistoryItem[]> = {};
      for (const wallet of BUYBACK_WALLETS) {
        try {
  ...
  const totalCurrentAssets = useMemo(() => {
    ...
    return BUYBACK_WALLETS.reduce((sum, wallet) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <header className="border-b border-slate-700/50 backdrop-blur-sm">
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-4 md:py-5">
          ...
          <h1 className="text-xl md:text-2xl font-bold text-white">바이백펀드 분석</h1>
        </div>
      </header>
