...
const KRILL_WALLET = '0x207Dd88810c115c8FDb6e2F2CD3245d5d1B86c51';
const PUMPSPACE_URL = `https://pumpspace.io/wallet/detail?account=${KRILL_WALLET}`;
...
export default function KrillAnalysisPage() {
  ...
  const { data: history, isLoading: historyLoading } = useQuery({
    queryKey: ['wallet-history', KRILL_WALLET],
    queryFn: () => fetchWalletHistory(KRILL_WALLET),
  });

  const { data: currentAssets } = useQuery({
    queryKey: ['wallet-current', KRILL_WALLET],
    queryFn: () => fetchCurrentAssets(KRILL_WALLET),
  });
      ...
      <header className="border-b border-slate-700/50 backdrop-blur-sm">
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-4 md:py-5">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-white mb-1">KRILL 펀드</h1>
              <code className="text-xs md:text-sm text-slate-400 font-mono">
                {formatAddress(KRILL_WALLET)}
              </code>
            </div>
            <a
              href={PUMPSPACE_URL}
