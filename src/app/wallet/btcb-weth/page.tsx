...
const PUMPSPACE_URL = `https://pumpspace.io/wallet/detail?account=${BTCB_WETH_WALLET}`;
...
export default function BtcbWethAnalysisPage() {
  ...
  const { data: history, isLoading: historyLoading } = useQuery({
    queryKey: ['wallet-history', BTCB_WETH_WALLET],
    queryFn: () => fetchWalletHistory(BTCB_WETH_WALLET),
  });

  const { data: currentAssets } = useQuery({
    queryKey: ['wallet-current', BTCB_WETH_WALLET],
    queryFn: () => fetchCurrentAssets(BTCB_WETH_WALLET),
  });
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <h1 className="text-xl md:text-2xl font-bold text-white">BTC.b - WETH.e</h1>
              </div>
              <code className="text-xs md:text-sm text-slate-400 font-mono">
                {formatAddress(BTCB_WETH_WALLET)}
              </code>
            </div>
            <a
              href={PUMPSPACE_URL}
